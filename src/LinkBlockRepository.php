...
class LinkBlockRepository
{
    ...
    public function getCMSBlocksSortedByHook($id_shop = null, $id_lang = null): array
    {
        ...
        $sql = 'SELECT
            ...
            FROM `' . _DB_PREFIX_ . 'link_block` bc
                INNER JOIN `' . _DB_PREFIX_ . 'link_block_shop` bcs 
                    ON (bc.`id_link_block` = bcs.`id_link_block`)
        ...
        foreach ($blocksSrc as $key => $block) {
            ...
            if ($block['id_lang'] == $id_lang && $block['block_name'] != ''){
                $blocks[$block['id_link_block']] = $block;
                unset($block);
            } else{
                $blocks[$block['id_link_block']] = $block;
            }
    ...
    public function getByIdHook($id_hook): array
    {
        ...
        $sql = "SELECT cb.`id_link_block`
                    ...
                    INNER JOIN {$this->db_prefix}link_block_shop cs ON (cb.`id_link_block` = cs.`id_link_block`)
